...
export default function TascoLayerSection() {
  ...
  return (
    <section className="bg-surface-container-low py-16 md:py-24 lg:py-32 px-4 sm:px-6 md:px-8" id="tasco">
      <div className="max-w-7xl mx-auto space-y-10 md:space-y-16">
        ...
        <div className="grid md:grid-cols-[280px_1fr] gap-6 md:gap-12 items-start">
          {/* Left: Layer Stack Navigation */}
          {/* Mobile: horizontal scroll, Desktop: vertical stack */}
          <div className="flex md:flex-col gap-2 sm:gap-3 overflow-x-auto md:overflow-x-visible pb-2 md:pb-0 -mx-4 px-4 sm:-mx-0 sm:px-0 scrollbar-hide">
            {layers.map((layer, idx) => (
              <button
                key={layer.id}
                onClick={() => setActiveLayer(layer.id)}
                className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 sm:py-4 rounded-xl sm:rounded-2xl text-left transition-all duration-300 flex-shrink-0 md:flex-shrink md:w-full ${
                  activeLayer === layer.id
                    ? `${layer.color} ${layer.textColor} ambient-shadow scale-[1.02]`
                    : 'bg-surface-container-lowest text-on-surface-variant hover:bg-surface-container'
                }`}
              >
                <span className="material-symbols-outlined text-xl sm:text-2xl flex-shrink-0">{layer.icon}</span>
                <div>
                  <div className="text-[10px] sm:text-xs font-bold uppercase tracking-widest opacity-70 hidden sm:block">
                    Layer {String(idx + 1).padStart(2, '0')}
                  </div>
                  <div className="font-bold text-xs sm:text-sm whitespace-nowrap">{layer.label}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          {/* Right: Layer Detail */}
          <div className="bg-surface-container-lowest rounded-2xl sm:rounded-3xl ambient-shadow p-5 sm:p-8 md:p-12 space-y-6 md:space-y-8">
            {/* Layer Title */}
            <div className="space-y-3 md:space-y-4">
              <div className="flex items-center gap-3 md:gap-4">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl ${active.color} ${active.textColor} flex items-center justify-center flex-shrink-0`}>
                  <span className="material-symbols-outlined text-xl sm:text-2xl">{active.icon}</span>
                </div>
                <div>
                  <div className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-0.5 sm:mb-1">
                    {active.label}
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
              {active.features.map((feature) => (
                <div
                  key={feature}
                  className="flex items-center gap-2 sm:gap-3 bg-surface-container-low rounded-xl px-3 sm:px-4 py-2.5 sm:py-3"
                >
                  <span
                    className="material-symbols-outlined text-tertiary text-base sm:text-lg flex-shrink-0"
                    style={{ fontVariationSettings: "'FILL' 1" }}
                  >
                    check_circle
                  </span>
                  <span className="text-xs sm:text-sm font-medium text-on-surface">{feature}</span>
                </div>
              ))}
            </div>

            {/* Architecture Diagram */}
            <div className="rounded-xl sm:rounded-2xl bg-surface-container-low p-4 sm:p-6 md:p-8">
              <div className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-4 sm:mb-6 text-center">
                ...
                {layers.map((layer, idx) => {
                  ...
                  return (
                    <div key={layer.id} className="flex flex-col items-center">
                      ...
                      >
                        <div
                          className={`relative flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-3 sm:py-4 rounded-lg sm:rounded-xl transition-all duration-300 cursor-pointer ${
                            isActive
                        ...
                        >
                          <div
                            className={`w-6 h-6 sm:w-8 sm:h-8 rounded-md sm:rounded-lg flex items-center justify-center text-[10px] sm:text-xs font-black flex-shrink-0 ${
                          ...
                          <div className="flex-1 text-left min-w-0">
                            <div className="text-xs sm:text-sm font-bold truncate">{layer.title}</div>
                          </div>

                          {isActive && (
                            <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-white/60 flex-shrink-0 animate-pulse" />
                          )}
                        </div>
                      </button>
                      ...
                      {idx < layers.length - 1 && (
                        <div className="flex flex-col items-center py-1 sm:py-1.5">
                          <div
                            className="w-px h-2 sm:h-3 transition-colors duration-300"
                            style={{ backgroundColor: isActive ? active.accentColor : 'rgba(0,81,201,0.2)' }}
                          />
                          <svg width="10" height="6" viewBox="0 0 12 8" className="transition-colors duration-300">
                            <path
                              d="M6 8L0 0h12z"
              ...
              <div className="flex items-center justify-center gap-2 mt-4 sm:mt-6 text-[10px] sm:text-xs text-on-surface-variant">
                <span className="material-symbols-outlined text-xs sm:text-sm">swap_vert</span>
